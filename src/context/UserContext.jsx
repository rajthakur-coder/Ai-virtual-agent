...
function UserContext({ children }) {
    ...
    async function aiResponse(prompt) {
        ...
        let newText = text.split("**") && text.split("*") && text.replace("google", "Raj") && text.replace("Google", "Raj")
        setPrompt(newText)
        speak(newText)
        setResponse(true)
        setTimeout(() => {
    ...
    function takeCommand(command) {
        if (command.includes("open") && command.includes("youtube")) {
            ...
            window.open('"C:\Users\DELL\Desktop\Visual Studio Code.lnk"');
            speak("Opening Visual Studio Code");
            setResponse(true);
            setPrompt("Opening VS Code...");
            setTimeout(() => {
                setSpeaking(false);
            }, 5000);
        }
